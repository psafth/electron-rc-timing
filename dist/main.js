...
(function (Mylaps) {
  ...
  var Long = __webpack_require__(/*! long */ "../mylaps-amb/node_modules/long/src/long.js");
  ...
  function (_super) {
    ...
    function Communicator() {
      var _this = _super.call(this) || this; // Escape character


      _this.ESC = 0x8d; // Value to subtract from escaped character (as integer)

      _this.ESCx = 0x20; // Start of record

      _this.SOR = 0x8e; // End of record
    ...
    Communicator.prototype.extractDataRows = function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (data[i] !== this.EOR) {
          ...
          var rowData = []; // Get the given amount of data from the record.

          for (var j = i + 2; j < i + lengthOfRecord + 2; j++) {
            // Store the value to the row
            if (i < data.length - 1 && j < data.length) rowData.push(data[j]);
    ...
    Communicator.prototype.unescape = function (dataToEscape) {
      ...
      var escapedData = []; // Iterate all the bytes, one by one.
    ...
    Communicator.prototype.getTimeMessageData = function (rawMessage) {
      var getTimeMessage = {}; // RTC_TIME = 0x01,
      // FLAGS = 0x04,
      // UTC_TIME = 0x05
      ...
      rawMessage.data.forEach(function (d) {
        switch (d.type) {
          case 0x01:
            ...
            var microseconds = Long.fromBytesLE(d.data); // Since js does not support 64-bit unsigned integers we have to neglect some resolution and go with milliseconds... :/

            var ms = microseconds.div(1000); // 1970-01-01

            var epoch = new Date(0); // Add the number of milliseconds since 1970-01-01

            epoch.setUTCMilliseconds(ms.toNumber()); // RTC
      ...
      rawMessage.data.forEach(function (d) {
        switch (d.type) {
          ...
          case 0x0c:
            ...
            var voltage = d.data.readInt8(0); // Convert to decimal. Ex. 11.9 is reported as 119.
      ...
      rawMessage.data.forEach(function (d) {
        switch (d.type) {
          ...
          case 0x10:
            ...
            epoch.setUTCMilliseconds(ms.toNumber()); // UTC
...
/***/ "../mylaps-amb/node_modules/long/src/long.js":
/*!***************************************************!*\
  !*** ../mylaps-amb/node_modules/long/src/long.js ***!
  \***************************************************/
/*! no static exports found */
...
/* harmony import */ var _mylaps_amb_dist__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../../mylaps-amb/dist */ "../mylaps-amb/dist/index.js");
/* harmony import */ var _mylaps_amb_dist__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_mylaps_amb_dist__WEBPACK_IMPORTED_MODULE_3__);
...
function createWindow() {
  ...
  var decoder = new _mylaps_amb_dist__WEBPACK_IMPORTED_MODULE_3__["Mylaps"].Communicator();
  decoder.on('connect', function (msg) {
    decoder.getTime(); // request the decoders rtc every 5 minutes.
  ...
  decoder.on('error', function (msg) {
    clearInterval(getTimeInterval);
    mainWindow.webContents.send('amb-disconnected', msg);
  });
  decoder.on('close', function (msg) {
    clearInterval(getTimeInterval);
    mainWindow.webContents.send('amb-disconnected', msg);
  });
